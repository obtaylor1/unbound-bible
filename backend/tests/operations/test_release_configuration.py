...
def test_frontend_image_builds_vite_and_serves_with_non_root_nginx():
    ...
    assert (
        "COPY --chmod=0755 frontend/15-dynamic-resolver.envsh "
        "/docker-entrypoint.d/15-dynamic-resolver.envsh"
    ) in dockerfile
    assert "RUN chmod 0755 /docker-entrypoint.d/15-dynamic-resolver.envsh" not in dockerfile
