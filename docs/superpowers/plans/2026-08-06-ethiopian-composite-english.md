...
Also assert all 108 Enoch chapters are nonempty, Enoch 80:1 exists, Enoch 22:2/22:6/27:3/32:1/32:3 contain only the Ethiopic main reading, Enoch 89:48 includes its `48^b.` fragment rather than attaching it to verse 49, the six WEB works match the official VPL chapter/verse positions, a second build is byte-identical, and no build input is modified.
...
For WEB works, parse `eng-webbe_vpl.txt` lines with a full-line expression equivalent to `^(BOOK) ([1-9][0-9]*):([1-9][0-9]*) (TEXT)$`; reject duplicates, unexpected books, and chapter/count drift. Preserve every official nonblank chapter/verse identity. Omit and declare Sirach's exact 24 explicit blank rows plus 12 numeric labels for which the VPL supplies no row; never renumber or invent placeholders. For Enoch, parse only the `The Book of Enoch` section of Project Gutenberg's official plain-text artifact, recognize Roman-numeral chapter headings I through CVIII and numbered verse/subverse markers such as `6.`, `6 a.`, `5 b.`, and `48^b.`. Use Charles's Ethiopic (`E`) main reading and exclude separately marked Greek alternate-recension blocks (`G^g`, `G^s`, `G^{s1}`, `G^{s2}`); never merge both readings into displayed prose. Join fragments carrying the same integer verse number in document order, discard Project Gutenberg boilerplate, presentation-only page numbers, and footnote blocks, preserve textual brackets and critical symbols, and reject missing/empty chapters, duplicate output positions, or a non-contiguous 1..N verse set. Do not use the damaged archive Enoch text in output. For Murdock, omit only the ten exactly reviewed blank alignment records, declare those positions, and replace four U+000F line separators across three verse texts with spaces; reject every other empty/control-bearing source text.

Write `corrected-bundle.zip` with sorted member names, UTF-8 canonical JSON, and fixed ZIP timestamps/permissions so repeated builds are byte-identical. Write `data-quality-report.json` containing every input/output checksum, raw findings, the frozen 38,938 corrected count, replacement counts, all 48 declared omissions, separate exact maps for the 24 explicit blank WEB rows and 12 absent WEB labels, and generator version. Never overwrite an input artifact.
...
All source records remain provisional. Mark archive-derived WMB and Murdock records modified because revision proof is absent and the archive documents name standardization. WMB's note covers numeric chapter ordering/name standardization only; Murdock's note also covers the ten blank alignment omissions and four U+000F-to-space normalizations across three texts. The six WEB records use the official eBible public-domain source URL; Sirach is marked modified solely to disclose omission/declaration of 24 explicit blank rows and 12 absent labels while preserving every nonblank official identity. Mark KJV fallback true. Attribute each Meqabyan work to Wikisource contributors under CC-BY-SA-4.0 using permanent revision URLs ending in oldids 16044809, 16044810, and 16044811; disclose that 2 Meqabyan 16:9 and 21:9 are absent upstream. Enoch uses Project Gutenberg ebook 77935 and is modified with a note that the Ethiopic main reading was selected, separately marked Greek alternate-recension blocks excluded, and subverse/layout fragments deterministically joined; Jubilees remains archive-derived with exact upstream revision unavailable.

The output uses edition EOTC-COMPOSITE-EN, English reading language, Mixed source language, Latin script, LicenseRef-Mixed, provisional source verification, general_reading relationship, composite adapter, prayer-of-manasseh as the sole supplemental work, and `known_missing_verses` exactly equal to 48 positions: 36 WEB Sirach positions, two 2 Meqabyan positions, and ten reviewed blank Murdock alignment positions. Its sole ingest `source_files` entry is `corrected-bundle.zip` with the checksum generated in the committed quality report.

Before reading coverage or emitting a manifest, `build_manifest.py` must hash the actual `corrected-bundle.zip` bytes and fail closed unless they exactly match `data-quality-report.json`. Tests include normal deterministic generation and a bundle with bytes appended after ZIP creation.
...
Expected: EOTC-COMPOSITE-EN, 83 works, 1,520 chapters, exactly 38,938 publishable rows. The adapter row count must equal that frozen count and `corrected_verse_count` in `data-quality-report.json`, with no duplicate position and no omission other than the 48 reviewed declarations (36 WEB Sirach, two 2 Meqabyan, ten Murdock alignment labels).
...
README records all four checksums (three inputs plus corrected output), distinguishes 44,114 raw records from exactly 38,938 publishable rows, records 82+1 scope, 13 unavailable works, distinct Enoch and Jubilees provenance records under the broad R. H. Charles source-family key, six KJV fallbacks, CC BY-SA duties and permanent revisions, the 36 WEB Sirach omissions split into 24 explicit blank rows and 12 absent labels, two upstream Meqabyan omissions, ten blank Murdock alignment labels, four U+000F normalizations across three texts, all modifications, provisional status, and exact operator commands. It must state that this is not an official or complete uniform Ethiopian Orthodox translation.
...
Expected: 38,938 corrected rows, zero errors, 83 source records, 82 Ethiopian scope, one supplemental, provisional status, and exactly 48 declared absent labels (36 WEB Sirach, two Meqabyan, ten Murdock alignment positions).
