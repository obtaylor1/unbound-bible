...
describe('commentary API', () => {
  ...
  it.each(['unknown', '', null, {}, 7])('uses no_entry for an invalid availability value', async (availability) => {
    ...
    expect(result.availability).toBe('no_entry')
  })
  ...
  it.each([
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ reference, entries: [] })))
    const result = await getCommentaryEntries({ source: 'john-gill', book: 'Genesis', chapter: 1 })
